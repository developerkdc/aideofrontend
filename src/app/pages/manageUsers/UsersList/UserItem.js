...
const UserItem = ({user}) => {
        ...
        switch (menuItem.action) {
            case 'edit':
                showDialog({
                    ...
                    content: <UserForm user={user} onSave={hideDialogAndRefreshContactsList}/>
                });
                break;
            case 'delete':
                sweetAlerts()
    ...
    const updateStatus = async () => {
        ...
        const message = user.status? "Blocked Successfully" : "Unblocked Successfully"
        Swal.fire({
            icon: "success",
            title: message,
            // text: "Done",
        ...
        <Card sx={{mb: 1}}>
            <Stack direction={"row"} alignItems={"center"} sx={{p: theme => theme.spacing(2, 1)}}>
                ...
                >
                    ...
                    <Typography variant={"body1"} color="text.secondary" sx={{alignItems: 'center'}}>Role</Typography>
                    <Typography variant={"h6"} mt={1} lineHeight={1.25} sx={{alignItems: 'center'}}>{(user.role).toUpperCase()}</Typography>
                ...
                >
                    <Stack spacing={25} direction={"row"} alignItems={"center"} sx={{textAlign: 'center'}}>
                        ...
                        <Typography variant={"body1"} color="text.secondary">Content</Typography>
                            <Typography variant={"h6"} mt={1} lineHeight={1.25}>{user.contentId.length}</Typography>
                            
                        </Item>
                        <Item>
                        <Typography variant={"body1"} color="text.secondary">Date</Typography>
                            <Typography variant={"h6"} mt={1} lineHeight={1.25}>{user.createdDate.substring(0,10)}</Typography>
                         </Item>
                    </Stack>
                </Item>
                <Item
                ...
                >
                    <Button sx={{minWidth: 92}} disableElevation variant={"contained"} size={"small"}
                            color={user.status ? "primary" : "error"} onClick={updateStatus}>
